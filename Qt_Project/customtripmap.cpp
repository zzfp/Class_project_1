...
customTripMap::customTripMap()
{
    ...
    for (int i=0;i<num_of_cities;i++)
    {
        ...
        query.prepare("SELECT Ending_City,Distance FROM distanceSheet INNER JOIN Custom_Trip WHERE Starting_City = (:cityname) AND Ending_City = City ORDER BY Distance");
...
void customTripMap::fullMap(QString start, int numberOfCities)
{
